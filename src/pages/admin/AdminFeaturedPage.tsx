  ...
  return (
    <motion.div
      ...
      className={`relative glass rounded-2xl overflow-hidden group ${
        isFeatured ? 'border border-yellow-500/25' : ''
      }`}
    >
      ...
      <div className="relative aspect-video overflow-hidden">
        <motion.img
          ...
          alt={v.title}
          className={`w-full h-full object-cover transition-opacity duration-300 ${isFeatured ? 'opacity-85 group-hover:opacity-100' : 'opacity-55 group-hover:opacity-70'}`}
        />
        {isFeatured && (
          <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}
            className="absolute top-2 right-2">
            <span className="glass text-yellow-400 text-xs font-bold px-2 py-0.5 rounded-full flex items-center gap-1 border border-yellow-500/30">
      ...
      {/* Footer */}
      <div className="p-3">
        <p className="text-white/80 text-xs sm:text-sm font-medium line-clamp-1 mb-2 group-hover:text-white transition-colors duration-200">
          {v.title}
        </p>
        <motion.button
          whileHover={{ scale: 1.03, y: -1 }} whileTap={{ scale: 0.96 }}
          onClick={onToggle}
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs transition-all w-full justify-center glass ${
            isFeatured
              ? 'text-red-400 border border-red-500/20 hover:bg-red-500/20'
              : 'text-yellow-400 border border-yellow-500/20 hover:bg-yellow-500/15'
          }`}
        >
          {isFeatured
  ...
  return (
    <div>
      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-xl sm:text-2xl font-bold text-white tracking-tight mb-1">Featured Videos</h1>
        <p className="text-white/40 text-sm mb-6 sm:mb-8">
          Featured videos appear in the homepage hero.{' '}
          <span className="text-yellow-400">{featured.length} currently featured.</span>
        </p>
      </motion.div>
      ...
      {featured.length === 0 ? (
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-white/30 text-sm mb-8">
